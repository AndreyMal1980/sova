...
<p>
    <strong>Прежде чем начать заполнять регистрационную форму Вам необходимо:</strong><br/><br/>
    1. Скачать заявление себе на компьютер(ссылка на скачивание ниже);<br/>
    2. Распечатать скачанное заявление;<br/>
    3. Заполнить и подписать заявление;<br/>
    4. Отсканировать и сохранить себе на компьютер заполненное заявление.<br/>
    5. С помощью кнопки "Выберите файл",которая находится в регистрационной форме ниже найти на Вашем компьютере<br/>
    ранее заполненное и сохраненное заявление и прикрепить его воспользовавшись кнопкой "Прикрепить скан заявление".<br/>
    6. Заполнить все регистрационные поля и нажать кнопку "Регистрация".<br/>
</p>

<?php
echo CHtml::link('скачать заявление', array('/download'));
?>
<br/><br/>
...
<div class="form">
    ...
    <div class="row">
        ...
        <?php echo $form->checkBoxList($model, 'programms',  CHtml::listData(Programms::model()->findAll(), 'programm_id', 'name')); ?>
    ...
    <div class="row">
        ...
        <?php echo $form->textField($model, 'number_ticket_change', array('size' => 60, 'maxlength' => 120,'readonly' => $model->getReadOnlyToRegistration($model->numberTicket,$model->readOnly))); ?>
